...
private:
    ...
    void update_empty_buckets(size_t index, typename List<NodeType, NodeAlloc>::iterator new_it) {
    ...
    void update_buckets() {
        ...
        for (auto it = elements.begin(); it != elements.end(); ++it) {
            size_t bucket_index = get_bucket_index(it->first);

            if (buckets[bucket_index].begin == elements.end()) {
                buckets[bucket_index].begin = it;
                buckets[bucket_index].end = std::next(it);
            }
            else {
                buckets[bucket_index].end = std::next(it);
            }
...
*/
	...
	struct PrepareResult {
        bool exists;
        size_t index;
        typename List<NodeType, NodeAlloc>::iterator it;
    };

    template <typename KeyType>
    PrepareResult prepare_key(const KeyType& key) {
        check_rehash();
        size_t idx = get_bucket_index(key);
        for (auto iter = buckets[idx].begin; iter != buckets[idx].end; ++iter) {
            if (key_equal(iter->first, key)) {
                return {true, idx, iter};
            }
        }
        return {false, idx, typename List<NodeType, NodeAlloc>::iterator()};
    }

    typename List<NodeType, NodeAlloc>::iterator place_in_bucket(size_t index, typename List<NodeType, NodeAlloc>::iterator it) {
        auto &bucket = buckets[index];
        if (bucket.begin == elements.end()) {
            update_empty_buckets(index, it);
            return it;
        } else {
            return elements.move_node(it, bucket.end);
        }
    }

    template <typename V>
    typename List<NodeType, NodeAlloc>::iterator insert_at_bucket(size_t index, V&& value) {
        auto new_it = elements.emplace(elements.end(), std::forward<V>(value));
        return place_in_bucket(index, new_it);
    }
...
*/
    ...
    std::pair<iterator, bool> emplace(Args&&... args) {
        ...
        auto placed_it = place_in_bucket(index, temp_it);
        return {iterator(placed_it), true};
    }
...
*/
	...
	std::pair<iterator, bool> insert(const NodeType& value) {
        auto prep = prepare_key(value.first);
        if (prep.exists) {
            return {iterator(prep.it), false};
        }
        auto new_it = insert_at_bucket(prep.index, value);
    ...
    std::pair<iterator, bool> insert(NodeType&& value) {
        ...
        auto new_it = insert_at_bucket(prep.index, std::move(value));
        return {iterator(new_it), true};
    }
